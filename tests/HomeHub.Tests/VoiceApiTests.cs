...
public class VoiceApiTests
{
    ...
    {
        using var app = new HubAppFactory();
        // Override the keyed local STT with an available stub (last keyed registration wins).
        // The endpoint itself touches no database, but since AUDIT A1 it needs a session like every
        // other endpoint — so the client signs in, which is the only reason the seeded roster
        // matters here.
