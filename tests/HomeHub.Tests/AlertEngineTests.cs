...
public class AlertEngineTests
{
    ...
    {
        var db = TestDb.New("alert-engine");
    ...
    public async Task Raises_alert_when_breach_is_sustained_past_the_duration()
    {
        ...
        var open = (await new AlertEngine().EvaluateAsync(db, now)).OpenCount;
    ...
    public async Task Clears_alert_once_the_condition_recovers()
    {
        ...
        await db.SaveChangesAsync();
        Assert.Equal(1, (await engine.EvaluateAsync(db, start)).OpenCount);
        ...
        var open = (await engine.EvaluateAsync(db, later)).OpenCount;
