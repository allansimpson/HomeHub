...
public class SensorsApiTests
{
    ...
    public async Task Zones_lists_seeded_zones_with_latest_reading()
    {
        ...
        // Named for the rooms the Climate zone table names, so its six rows bind to real probes.
        Assert.Equal(
            new[] { "Freezer", "Fridge", "Living Room", "Kitchen", "Master Bedroom", "Upstairs Office" },
            zones.Select(z => z.Name));
    ...
    public async Task Thresholds_list_and_update()
    {
        ...
        Assert.Equal(6, thresholds!.Count);
