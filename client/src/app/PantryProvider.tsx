...
interface PantryState {
  ...
  updateItem: (id: number, input: PantryItemInput, baseVersion?: number) => Promise<void>
...
export function PantryProvider({ children }: { children: ReactNode }) {
  ...
  const undoEvent = useCallback(async (eventId: number) => {
    await api.undoPantryEvent(eventId, activeProfileId)
    await refresh()
  ...
  const scan = useCallback(async (input: Omit<ScanInput, 'profileId'>) => {
    const result = await api.scanIntoPantry({ ...input, profileId: activeProfileId })
    await refresh()
    return result
  }, [refresh, activeProfileId])
  ...
  const addManyToGrocery = useCallback(async (lines: GroceryInput[]) => {
    await run({
      ...
      method: 'POST',
      path: '/grocery/batch',
      body: { lines: lines.map((l) => ({ ...l, profileId: l.profileId ?? activeProfileId })) },
  ...
  const checkGrocery = useCallback(async (id: number, checkedOff: boolean) => {
    ...
    await run({
      domain: 'grocery',
      method: 'POST',
      path: `/grocery/${id}/check?checkedOff=${checkedOff}${activeProfileId != null ? `&profileId=${activeProfileId}` : ''}`,
