...
interface NotificationsState {
  ...
  unreadCount: number
  /**
   * Unread *wants you* items only — what the account avatar's badge counts.
   *
   * **Never the total.** The badge means "things that want a human"; worth-knowing items wait in the
   * inbox and never produce one. Two consequences that are correct rather than bugs: the badge can
   * read `1` while Config's row reads `5 WAITING` (the row is the seven-day record, the badge is the
   * demand), and three live cards can be on screen with the avatar unbadged if all three are
   * worth-knowing.
   *
   * Household-scoped, like everything else here — the queue belongs to the house, not to whoever is
   * signed in, so switching profile does not change the number. That falls out of the store already:
   * `GET /api/notifications` takes no profile.
   */
  wantsYouCount: number
  drawerOpen: boolean
...
export function NotificationsProvider({ children }: { children: ReactNode }) {
  ...
  // Counted off `all` rather than tracked alongside `unreadCount`, so it cannot drift from the list
  // it describes — `markRead` already updates `all` optimistically, and this follows for free.
  const wantsYouCount = useMemo(
    () => all.filter((n) => n.severity === 'wants-you' && !n.read).length,
    [all],
  )

  const value = useMemo<NotificationsState>(
    () => ({
      all, live, unreadCount, wantsYouCount, drawerOpen, sources, loading,
      dismiss, markRead, clearAll, setSource, refresh,
      openDrawer: () => setDrawerOpen(true),
      closeDrawer: () => setDrawerOpen(false),
    }),
    [all, live, unreadCount, wantsYouCount, drawerOpen, sources, loading, dismiss, markRead, clearAll, setSource, refresh],
  )
