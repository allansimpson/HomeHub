...
export function ImportScreen() {
  ...
  useEffect(() => {
    ...
    void api.getImport(Number(id))
      ...
      .catch(() => { if (!cancelled) navigate('/meals/pantry', { replace: true }) })
  ...
  const parse = async () => {
    ...
    try {
      ...
      setImported(created)
      navigate(`/meals/pantry/import/${created.id}`, { replace: true })
    } finally {
      setBusy(false)
    }
  }
  ...
  const apply = async () => {
    ...
    try {
      ...
      await refresh()
      navigate('/meals/pantry', { replace: true })
    } catch (err) {
        ...
        footer={
          <div className="pt-modal__foot">
            <div className="pt-footer__row">
              ...
              <SecondaryButton onClick={() => navigate('/meals/pantry')}>NOT NOW</SecondaryButton>
            </div>
          </div>
        }
      >
        ...
        <div className="pt-modal__foot">
          {taken || alreadyApplied ? (
            <>
              ...
              <PrimaryButton onClick={() => navigate('/meals/pantry')}>SEE THE PANTRY</PrimaryButton>
            </>
          ) : (
            <>
              <div className="pt-footer__row">
...
function ImportLineSheet({
  ...
  const save = async () => {
    ...
    try {
      ...
      // A corrected line can introduce a unit the list has never seen — offer it on the next one.
      refreshUnits()
      onSaved(next)
    } finally {
      setBusy(false)
