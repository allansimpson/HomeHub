...
export function RecipeDetailScreen() {
  ...
  const { settings, week, recipes, refresh } = useMeals()
  ...
  /**
   * The household's list, plus whatever this recipe already says.
   *
   * A page can import as `cuisine:cajun` when Cajun is not on the household's list, and offering
   * only the canonical names would make the picker unable to show — let alone keep — the value the
   * chip is currently displaying. Adding new names to the list itself stays in Meals settings,
   * which is where the folder's headings are managed.
   */
  const cuisineOptions = cuisine && !settings.canonicalCuisines.includes(cuisine)
    ? [cuisine, ...settings.canonicalCuisines]
    : settings.canonicalCuisines

  const chooseCuisine = async (name: string | null) => {
    setPickingCuisine(false)
    try {
      // The server answers with the saved recipe, so the chip re-renders from what was stored
      // rather than from what was tapped — the two agree, or the screen shows the disagreement.
      setRecipe(await api.setRecipeCuisine(recipe.id, name, recipe.version))
      // The folder groups by this, so its copy is stale the moment this returns.
      void refresh()
    } catch (err) {
      // Offline, or somebody else edited the recipe first. Re-read rather than guess: a chip
      // showing a cuisine the server refused would be the one lie this screen must not tell.
    ...
    <ScreenShell header={<DrillInHeader title={recipe.title} onBack={() => navigate(-1)} />}>
      <ScrollArea>
        <div className="ml-recipe__meta">
          {/*
            Tappable, because the importer's guess is only ever a guess: `recipeCuisine` is whatever
            the publisher typed, plenty of pages omit it entirely, and the folder groups by this. A
            chip nobody can change is a wrong heading for ever.

            It reads `+ CUISINE` when there is none — the uncategorised case needs a way in as much
            as the wrong-category one, and a recipe with no chip at all would otherwise offer no
            target to tap.
          */}
          <button
            type="button"
            className={'ml-recipe__cuisine' + (cuisine ? '' : ' ml-recipe__cuisine--unset')}
            aria-expanded={pickingCuisine}
            aria-label={cuisine ? `Cuisine: ${cuisine}. Change it.` : 'Set a cuisine'}
            onClick={() => setPickingCuisine((open) => !open)}
          >
            {cuisine ? cuisine.toUpperCase() : '+ CUISINE'}
          </button>
          <span className="ml-recipe__times">{timesLine(recipe)}</span>
        </div>

        {pickingCuisine && (
          <div className="ml-add__chips ml-recipe__cuisines" role="listbox" aria-label="Cuisine">
            {cuisineOptions.map((name) => {
              ...
              return (
                <button
                  ...
                  role="option"
                  aria-selected={on}
                  className={'ml-add__chip' + (on ? ' ml-add__chip--active' : '')}
                  // Tapping the one it already is clears it. Same gesture as the add screen, and it
                  // is the only way back to uncategorised once a chip has been set.
                  onClick={() => void chooseCuisine(on ? null : name)}
                >
                  {name.toUpperCase()}
                </button>
              )
