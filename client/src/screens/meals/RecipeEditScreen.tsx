          ...
          <div className="ml-edit__lines">
            {lines.map((line) => (
              <div className="ml-edit__row" key={line.id}>
                ...
                <span className="ml-edit__unit">{line.unit.toUpperCase() || '—'}</span>
            ...
            return (
              <div key={line.id}>
                ...
                <div className="ml-edit__row">
                  ...
                  <button
                    type="button"
                    className="ml-edit__unit"
                    onClick={() => setLines((prev) => prev.map((l) => (l.id === line.id ? { ...l, unit: nextUnit(l.unit) } : l)))}
                  >
...
/** The units a household actually writes. Free text is still allowed via the importer. */
const UNITS = ['', 'g', 'kg', 'ml', 'l', 'tsp', 'tbsp', 'cup', 'oz', 'lb', 'ea', 'clove', 'pinch']

/** Tapping the unit cycles the list — a picker for thirteen options is a menu nobody needs. */
function nextUnit(current: string): string {
  const at = UNITS.findIndex((u) => u.toLowerCase() === current.toLowerCase())
  return UNITS[(at + 1) % UNITS.length]
}
