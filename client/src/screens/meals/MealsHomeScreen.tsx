...
export function MealsHomeScreen() {
  ...
  const confirmAte = async (night: MealPlanEntryDto) => {
    ...
    try {
      const receipt = await api.deductForNight(night.id)
      if (receipt) navigate(`/meals/pantry/taken/${night.id}`)
    } catch {
  ...
  return (
    <ScreenShell header={<DrillInHeader title="MEALS" status={clock} />}>
      {/* Pantry joined the section as a third segment rather than keeping its own tab (IA.md). The
          control sits above the scroll so it stays put while the week scrolls under it. */}
      <MealsSegment active="week" />
      <ScrollArea>
