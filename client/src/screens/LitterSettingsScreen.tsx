...
export function LitterSettingsScreen() {
  ...
  const header = (
    // Back goes to Config, not to Care: this is a Config sub-page now, reached from the Devices
    // group, and returning to the section it used to belong to would be a dead end.
    <DrillInHeader title="Litter Settings" onBack={() => navigate('/settings')} status={robot?.name ?? ''} />
  )
