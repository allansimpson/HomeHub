...
public class HomeHubDbContextFactory : IDesignTimeDbContextFactory<HomeHubDbContext>
{
    public HomeHubDbContext CreateDbContext(string[] args)
    {
        ...
        // An ephemeral key ring, because this context only ever builds a model to diff against.
        // `dotnet ef` scaffolds and applies *schema*; it never reads or writes a credential column,
        // so the protector here needs to satisfy the constructor and nothing more. Reaching for the
        // real key directory would be worse than useless — it would make authoring a migration
        // depend on the production key material being present on the developer's machine.
        return new HomeHubDbContext(options, new SecretProtector(new EphemeralDataProtectionProvider()));
    }
}
