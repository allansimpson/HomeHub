...
public sealed class OpenFoodFactsProductLookup : IProductLookup
{
    ...
    internal static ProductInfo? ToProductInfo(OffResponse? payload)
    {
        ...
        // Cased to the house style before it is ever shown. The field is whatever a contributor
        // typed, so the same catalogue answers `TRADITIONAL ITALIAN POLENTA` for one product and
        // `Traditional italian polenta` for the next; on a shelf list read from across a room, that
        // inconsistency is the loudest thing on the screen. Still only a suggestion — whatever is in
        // the box when NAME IT is pressed is what the household gets.
        return new ProductInfo(
            ProductNames.TitleCase(Truncate(name, PantryFieldLimits.ItemName))!,
            brand is null ? null : ProductNames.TitleCase(Truncate(brand, PantryFieldLimits.ItemName)),
