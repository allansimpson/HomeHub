...
public sealed class StockCheckService
{
    ...
    internal static StockStatus Verdict(PantryItem item, RecipeIngredient ingredient, decimal factor)
    {
        ...
        // Five 3 oz pots is fifteen ounces. The multiplication happens in one place so a packaged row
        // and a loose one cannot be read differently by different callers — see PantryAmounts.
        var have = PantryAmounts.OnHand(item);
        ...
        var comparable = UnitConversion.Convert(scaled, ingredient.Unit, PantryAmounts.MeasureUnit(item));
