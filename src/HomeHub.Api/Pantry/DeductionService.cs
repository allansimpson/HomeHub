...
public sealed class DeductionService
{
        ...
        PantryItem item, RecipeIngredient ingredient, decimal factor, int? byProfileId, int planEntryId)
    {
        ...
        var comparable = scaled is null ? null : UnitConversion.Convert(scaled.Value, ingredient.Unit, item.Unit);

        if (comparable is { } amount) return Write(-amount, null);
        ...
        foreach (var e in events.Where(e => e.Kind == PantryEventKind.Deducted))
        {
            ...
            if (e.Delta is 0)
            {
                estimated.Add(new ReceiptLineDto(
                    ...
                    $"the recipe's units don't convert to {item.Unit ?? "what's on the shelf"} — nothing counted off",
                    undone));
                continue;
            }
