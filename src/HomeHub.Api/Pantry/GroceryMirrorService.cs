...
public sealed class GroceryMirrorWorker : BackgroundService
{
    ...
    private readonly TimeProvider _clock;
    private readonly ILogger<GroceryMirrorWorker> _logger;

    public GroceryMirrorWorker(
        GroceryMirrorService mirror, TimeProvider clock, ILogger<GroceryMirrorWorker> logger)
    {
        _mirror = mirror;
        _clock = clock;
        _logger = logger;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (await timer.WaitForNextTickAsync(stoppingToken))
        {
            ...
            try
            {
                ok = await _mirror.SyncAsync(stoppingToken);
            }
            ...
            {
                // **A grocery-list sync must never take the panel down.** It could: an unhandled
                // exception out of ExecuteAsync stops the whole host by default, and this loop was
                // the one worker here without a guard. On the server that turned a stale schema into
                // a crash loop — the host stopped, systemd restarted it three seconds later, the
                // dying process still held :5000, the bind failed, and 218 restarts later the
                // climate loop, the calendar and the litter box were all down over a shopping list.
                //
                // Backed off like a failed sync rather than retried immediately: whatever broke is
                // unlikely to be fixed by asking again a second later.
                _logger.LogError(ex, "Grocery mirror sync failed; backing off and retrying.");
                ok = false;
            }
