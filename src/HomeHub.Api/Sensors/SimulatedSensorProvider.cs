...
public sealed class SimulatedSensorProvider : ISensorProvider, ISensorHistoryBackfill
{
    ...
    private static readonly IReadOnlyDictionary<string, Spec> Zones = new Dictionary<string, Spec>
    {
        ...
        ["sim-bedroom"] = new("Master Bedroom", SensorCategory.Ambient, BaseTemp: 70, TempSwing: 3, BaseHumidity: 46, HumiditySwing: 5),
        // Deliberately the warm room. Its unit is already at the bottom of its range in the seed, so
        // without hardware the panel still shows one room the loop genuinely cannot hold — the state
        // that is easiest to get wrong and hardest to reproduce on demand.
        ["sim-office"] = new("Upstairs Office", SensorCategory.Ambient, BaseTemp: 76, TempSwing: 3, BaseHumidity: 48, HumiditySwing: 5),
    };
