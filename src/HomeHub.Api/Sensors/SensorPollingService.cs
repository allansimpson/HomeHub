...
public sealed class SensorPollingService : BackgroundService
{
    ...
    private async Task PollOnceAsync(CancellationToken ct)
    {
        ...
        // 4. Reconcile alerts against the fresh readings, and tell the household about the new ones.
        var pass = await engine.EvaluateAsync(db, now, ct);
        await NotifyAsync(scope, db, zones, pass, ct);
    }

    /// <summary>
    /// Turn newly-raised sensor alerts into notifications.
    /// </summary>
    /// <remarks>
    /// The gap this closes: nothing converted sensor alerts into notifications, so a freezer warming
    /// up raised a row on the Dashboard and reached the household in no other way — the panel had to
    /// be looked at for the warning to exist. Cold storage is the case that makes that untenable,
    /// because the whole point of watching a freezer is being told when nobody is watching it
    /// (CLIMATE_BEHAVIOURS §7).
    /// <para>
    /// Only the transitions, never the open set: an alert that has been true for an hour has already
    /// been reported, and the dedupe key carries the alert's own identity so a restart mid-breach
    /// cannot report it twice.
    /// </para>
    /// </remarks>
    private static async Task NotifyAsync(
        IServiceScope scope, HomeHubDbContext db, List<SensorZone> zones, Alerts.SensorAlertPass pass, CancellationToken ct)
    {
        if (pass.Raised.Count == 0) return;
        var notifications = scope.ServiceProvider.GetService<NotificationService>();
        if (notifications is null) return;

        var byId = zones.ToDictionary(z => z.Id);
        foreach (var alert in pass.Raised)
        {
            var zoneId = int.TryParse(alert.Source.Split(':').ElementAtOrDefault(1), out var id) ? id : 0;
            var zone = byId.GetValueOrDefault(zoneId);
            await notifications.RecordAsync(
                source: Notifications.NotificationSources.Climate,
                label: zone?.Name.ToUpperInvariant() ?? "HOUSE",
                // Food safety is the one thing here that wants a person to get up. An ambient
                // humidity ceiling is worth knowing and nothing more.
                severity: zone?.Category == SensorCategory.FoodSafety
                    ? Notifications.NotificationSeverities.WantsYou
                    : Notifications.NotificationSeverities.WorthKnowing,
                accent: alert.Severity == Alerts.AlertSeverity.Severe ? "terracotta" : "brass-bright",
                headline: alert.Message,
                dedupeKey: $"alert:{alert.Type}:{alert.DedupeKey}:{alert.StartedAtUtc:O}",
                atUtc: alert.StartedAtUtc,
                route: $"/sensor?zone={zoneId}",
                ct: ct);
        }
    }
