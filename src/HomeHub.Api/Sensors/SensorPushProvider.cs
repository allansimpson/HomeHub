...
/// </summary>
public sealed class SensorPushProvider : ISensorProvider, IDisposable
{
    ...
    /// <summary>Releases the token-refresh lock's wait handle.</summary>
    /// <remarks>
    /// This is a scoped service behind <c>AddHttpClient&lt;SensorPushProvider&gt;()</c>, so one
    /// instance is built per request that touches sensors — and the panel polls every 30 seconds,
    /// forever. Each instance's <see cref="SemaphoreSlim"/> takes an unmanaged wait handle the first
    /// time it is awaited, and without this they accumulate for the life of the process. Declaring
    /// <see cref="IDisposable"/> is enough: the DI container disposes scoped services it created.
    /// </remarks>
