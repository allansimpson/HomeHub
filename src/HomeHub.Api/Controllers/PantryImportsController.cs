...
public class PantryImportsController : ControllerBase
{
    ...
    public PantryImportsController(HomeHubDbContext db, PantryLedger ledger, TimeProvider clock)
    {
        _db = db;
        _ledger = ledger;
        _clock = clock;
        ...
        foreach (var parsed in OrderImportParser.Parse(input.RawPayload))
        {
            ...
            else
            {
                ...
                if (key.Length > 0 && byKey.TryGetValue(key, out var match))
                {
                    ...
                    line.ProposedUnit = match.Unit ?? parsed.Unit;
        ...
        int id, int lineId, ImportLineInput input, CancellationToken ct)
    {
        ...
        if (input.ProposedUnit is not null)
            line.ProposedUnit = string.IsNullOrWhiteSpace(input.ProposedUnit) ? null : input.ProposedUnit.Trim();
    ...
    public async Task<ActionResult<OrderImportDto>> Apply(int id, ApplyImportInput input, CancellationToken ct)
    {
        ...
        foreach (var line in import.Lines.OrderBy(l => l.Position))
        {
            ...
            _ledger.Record(
                item, PantryEventKind.Imported, input.ProfileId,
        ...
        import.AppliedByProfileId = input.ProfileId;
    ...
    public async Task<IActionResult> Undo(int id, [FromQuery] int? profileId, CancellationToken ct)
    {
