...
public class GroceryController : ControllerBase
{
    ...
    public GroceryController(
        HomeHubDbContext db, PantryLedger ledger, GroceryMirrorService mirror, TimeProvider clock)
    {
        _db = db;
        _ledger = ledger;
        _mirror = mirror;
    ...
    public async Task<ActionResult<GroceryLineDto>> Check(
        int id, [FromQuery] bool checkedOff = true, [FromQuery] int? profileId = null,
    ...
    private async Task<GroceryLine> MergeAsync(GroceryInput input, CancellationToken ct)
    {
        ...
        if (line is null)
        {
            line = new GroceryLine
            {
                ...
                CreatedUtc = now,
                AddedByProfileId = input.ProfileId,
                MirrorPending = true,
            };
