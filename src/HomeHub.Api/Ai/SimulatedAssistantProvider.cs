...
public sealed class SimulatedAssistantProvider : IAssistantProvider
{
    ...
    private static string Answer(AssistantRequest request)
    {
        ...
        if (p.StartsWith("set ") || p.Contains("turn on") || p.Contains("turn off") || p.Contains("degrees"))
            return "I can't control climate yet — adjust zones on the Climate screen. Add an OpenAI key for more spoken control.";
        if (p.StartsWith("add ") || p.Contains(" list") || p.Contains("task"))
            return "To add to a list, say “add <item> to <list>” and I'll do it. For anything else, add an OpenAI key.";
        ...
        return $"I'm Barnaby, the on-device demo assistant. Configure a local model (Ai:LocalEndpoint) or an OpenAI key (Ai:OpenAiApiKey) for full answers. You asked: “{request.Prompt.Trim()}”.";
    }
}
