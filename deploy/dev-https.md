...
Get `certs/homehub-dev-ca.crt` onto the device. The least painful route is to let the dev server
hand it over — copy it into `client/public/` (gitignored) and browse to it from the phone:

```bash
cp certs/homehub-dev-ca.crt client/public/
# then on the phone:  https://<dev-machine-ip>:5173/homehub-dev-ca.crt
```

The download itself will show a certificate warning, since the phone does not trust the CA yet —
that is expected and harmless: the file is public by design, and nothing sensitive crosses.

**Delete it from `client/public/` once the devices are set up.** `npm run build` copies that folder
into `src/HomeHub.Api/wwwroot/`, so leaving it there means every production build serves a
development CA from the panel's own web root. It is only a public certificate, not the key, but it
is dev config with no business being in a deployed artifact — and the file is one command to
regenerate when a new device needs it.
...
**None of this applies to the deployed panel.** `deploy/server-systemd.md` sets
`ASPNETCORE_URLS=http://0.0.0.0:5000` and the kiosk browses to it over the LAN; the certificate
code is gated on `IsDevelopment()` and no certificate is ever generated there.

If phone scanning is wanted against the *real* panel rather than a dev machine, that host needs its
own TLS — the same local-CA approach, or a proper certificate via a reverse proxy. Worth deciding
before the pantry is relied on, since scanning a delivery is a phone job by definition.
